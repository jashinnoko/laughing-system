--[[
    KAZU DUELS  BY @KaZuDev and ghost  FIXED Bugs
...
--  CONFIG
-- ============================================================
local CONFIG_FILE = "KaZuDuels_config.json"
...
local function makeWpMarker(pos,label,col)
    ...
    part.Name="KaZu_WpMarker"; part.Shape=Enum.PartType.Ball
...
local function saveCustomWaypoints()
    pcall(function()
        ...
        writefile("KaZuDuels_waypoints.json",game:GetService("HttpService"):JSONEncode(data))
    end)
end

local function loadCustomWaypoints()
    pcall(function()
        if not readfile or not isfile or not isfile("KaZuDuels_waypoints.json") then return end
        local raw=readfile("KaZuDuels_waypoints.json")
...
local function CreateDiscordLabel()
    ...
    dcLabel.Text = "discord.gg/6NxGAWrbs"
...
local function ApplyTheme(name)
    ...
    task.spawn(function()
        ...
        pcall(function() ParentUI:FindFirstChild("KaZuDuels"):Destroy() end)
        BuildMainGUI()
    end)
end
...
pcall(function()
    if readfile and isfile and isfile("KaZuDuels_btnpos.json") then
        local raw=readfile("KaZuDuels_btnpos.json")
        if raw and raw~="" then
            local data=game:GetService("HttpService"):JSONDecode(raw)
...
BuildMainGUI = function()
    ...
    Screen.Name="KaZuDuels"; Screen.ResetOnSpawn=false
    ...
    TitleLbl.BackgroundTransparency=1; TitleLbl.Text="KAZU DUELS"
            ...
            if t2.taunt      then
                ...
                tauntLoop=task.spawn(function()
                    while tauntActive do
                        pcall(function() local ch=game:GetService("TextChatService").TextChannels:FindFirstChild("RBXGeneral"); if ch then ch:SendAsync("/kazu hub") end end)
                        task.wait(0.5)
                    end
                end)
            end
    ...
    if isMobile then
        ...
        savePosBtn.MouseButton1Click:Connect(function()
            ...
            pcall(function()
                ...
                writefile("KaZuDuels_btnpos.json",game:GetService("HttpService"):JSONEncode({btns=btnData,main=mainData}))
            end)
        ...
        local function dispatchToggle(id)
            ...
            elseif id=="taunt" then
                ...
                if tauntActive then tauntLoop=task.spawn(function() while tauntActive do pcall(function() local ch=game:GetService("TextChatService").TextChannels:FindFirstChild("RBXGeneral"); if ch then ch:SendAsync("/kazu hub") end end); task.wait(0.5) end end)
        ...
        globalKeybindConn = UIS.InputBegan:Connect(function(inp,gpe)
            ...
            for id,kc in pairs(FeatureKeybinds) do
                if inp.KeyCode==kc then
                    ...
                    elseif id=="steal" then stealActive=not stealActive; ssSt(stealActive); FeatureStates.steal=stealActive; if stealActive then startStealLoop() else stopStealLoop() end
                    elseif id=="stealSpd" then stealSpeedActive=not stealSpeedActive; ssSspd(stealSpeedActive); FeatureStates.stealSpd=stealSpeedActive; if stealSpeedActive then startStealSpeed() else stopStealSpeed() end; SaveConfig()
                    elseif id=="taunt" then tauntActive=not tauntActive; ssTnt(tauntActive); FeatureStates.taunt=tauntActive; if tauntActive then tauntLoop=task.spawn(function() while tauntActive do pcall(function() local ch=game:GetService("TextChatService").TextChannels:FindFirstChild("RBXGeneral"); if ch then ch:SendAsync("/kazu hub") end end); task.wait(0.5) end end) else if tauntLoop then task.cancel(tauntLoop); tauntLoop=nil end end; SaveConfig()
                    elseif id=="autoGrab" then autoGrabActive=not autoGrabActive; ssGrab(autoGrabActive); FeatureStates.autoGrab=autoGrabActive; if autoGrabActive then startAutoGrab() else stopAutoGrab() end; SaveConfig()
                    elseif id=="optimizer" then optimizerActive=not optimizerActive; ssOpt(optimizerActive); FeatureStates.optimizer=optimizerActive; if optimizerActive then enableOptimizer() else disableOptimizer() end; SaveConfig()
    ...
    print("[KaZu Duels]: "..(isMobile and "Mobile" or "PC").." | Scale:"..guiScale.." | Theme:"..T.name)
end
...
SelTitle.BackgroundTransparency=1; SelTitle.Text="KAZU DUELS"
...
getgenv().StopKaZuDuels=function()
